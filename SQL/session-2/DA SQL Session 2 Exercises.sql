...
rename column feedback_text to feedback_comment;

alter table customer_feedback
rename column feedback_comment to feedback_text;
...
-- SAME THING AS 
-- update customer_invoice
-- set due_date = str_to_date(due_date, '%d/%m/%Y');
-- alter table customers_invoice
...
-- TO SEE HOW A TABLE WOULD LOOK LIKE AFTER A CERTAIN CHANGE IS MADE. WE WILL USE START TRANSACTION.

START transaction;
delete from customer_invoice
where amount_due = 0;
rollback;
-- NOW I WANT TO REVERSE THE CHANGE IN THE TRANSACTION I WILL SIMPLY USE ROLLBACK
-- WORKS ONLYYYYY FOR DELETION, UPDATION AND INSERTION.